...
class TrackEmail {
    ...
    {
		$email_id = $GLOBALS['wpdb']->get_var( $GLOBALS['wpdb']->prepare('SELECT trklink_email_id FROM '.emtr_get_table_name( 'track_email_link_master'  ).' WHERE trklink_id=%d', $link_id) );
		self::insert_link_open_log( $email_id );
		$GLOBALS['wpdb']->insert(
					emtr_get_table_name( 'track_email_link_click_log' ),
					array (
						'trklinkclick_date_time' => gmdate('Y-m-d H:i:s'),
						'trklinkclick_http_user_agent'	=> $_SERVER["HTTP_USER_AGENT"],
	...
	public function insert_link_open_log( $email_id ) {
		...
		$sql =  $GLOBALS['wpdb']->prepare('SELECT count(*) as count FROM '.emtr_get_table_name( 'track_email_open_log' ).' WHERE trkemail_email_id=%d AND trkemail_ip_address = %s AND trkemail_date_time > %s', $email_id, $client_ip, $interval_date_time );
    ...
    public function email_link_replace($email_content, $email_id, $user_id)
    {
        ...
        {
			$GLOBALS['wpdb']->insert( 
					emtr_get_table_name( 'track_email_link_master' ),  
					array( 
						'trklink_link' => $link, 
