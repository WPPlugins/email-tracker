...
class EMTR_Email_List_Table extends WP_List_Table {
    ...
    function __construct(){
        ...
        parent::__construct( array(
            'singular'  => __('email', EMTR_TEXT_DOMAIN),     //singular name of the listed records
            'plural'    => __('emails', EMTR_TEXT_DOMAIN),    //plural name of the listed records
    ...
    function column_default($item, $column_name){
        switch($column_name){
			...
			case 'click_count':
				$str = '<b>'.sprintf( _n( '%s time', '%s times', $item[$column_name] ), $item[$column_name] ).__(' clicked',EMTR_TEXT_DOMAIN).'</b>';
				if( $item[$column_name] == 0 ) {
					$str .= '<div alt="f147" class="dashicons dashicons-no-alt"></div>';
				}
				else {
				...
				if( !empty($item['click_date_time']) ) {
					...
					$str .= __('Last click on ', EMTR_TEXT_DOMAIN). get_date_from_gmt($arr_click_date_time[0], 'F j, Y g:i A').'&nbsp;'.emtr_relative_time(get_date_from_gmt( $arr_click_date_time[0] ));
    ...
    function column_to($item){
        ...
        $actions = array(
            ...
            'delete'    => sprintf('<a href="?page=%s&action=%s&email=%s" class="delete">Delete</a>',$_REQUEST['page'],'delete',$item[$this->key]),
        );
    ...
    function process_bulk_action() {
        ...
        if( 'delete'===$this->current_action() ) {
			...
			if( count($arr_email_id) > 0 ) {
				...
				$wpdb->query( 
						'DELETE FROM '.$this->table_name.' WHERE '.$this->key.' IN ('.implode(',',$arr_email_id) .')'
					);
					
				$wpdb->query(
						'DELETE FROM '.$this->open_log_table_name.' WHERE trkemail_email_id IN ('.implode(',',$arr_email_id).')'
						);
				
				$wpdb->query(
					'DELETE FROM '.
						emtr_get_table_name( 'track_email_link_click_log' ).' WHERE 	trklinkclick_trklink_id IN (SELECT trklink_id FROM '.emtr_get_table_name( 'track_email_link_master' ).' WHERE trklink_email_id IN ('.implode(',',$arr_email_id).') 
					)'	
				);
				
				$wpdb->query(
						'DELETE FROM '.emtr_get_table_name( 'track_email_link_master' ).' WHERE trklink_email_id IN ('.implode(',',$arr_email_id).')'
					);
